	...
	ast.Inspect(astFile, func(n ast.Node) bool {
		...
		case *ast.CompositeLit:
			...
			if r.enforceSliceStyle == enforceSliceStyleTypeNil {
				failureMessage = "use nil slice (e.g. var args []type or []type(nil)) instead of []type{}"
			} else {
		...
		case *ast.CallExpr:
			...
			var failureMessage string
			if r.enforceSliceStyle == enforceSliceStyleTypeNil {
				failureMessage = "use nil slice (e.g. var args []type or []type(nil)) instead of make([]type, 0)"
			} else {
